  ...
  <div class="row h-100 m-0 p-0">
    <div class="col-3 h-100 m-0 p-0 border-left border-right border-gray">
      ...
      <div class="left-memo-list list-group-flush p-0">
        ...
        <a href="{{ route('memo.select',['id' => $memo->id]) }}" class="list-group-item list-group-item-action @if($select_memo) {{ $select_memo->id == $memo->id ? 'active' : '' }}@endif">
          ...
          <p class="mb-1">
            @if(mb_strlen($memo->content) <= 100) {{ $memo->content}} @else {{ mb_substr($memo->conteny, 0, 100). "..."}} @endif </p>
        </a>
        @endforeach
      </div>
    </div>
    <div class="col-9 h-100">
      ...
      <form class="w-100 h-100" method="post">
        ...
        <div id="memo-menu">
          <button type="submit" class="btn btn-danger" formaction=""><i class="fas fa-trash-alt" formaction=""></i></button>
        ...
        <textarea name="edit_content" id="memo-content" placeholder="内容を入力する...">{{ $select_memo->content }}</textarea>
      </form>
      @else
